...
layout(location = 15) uniform struct Fog {
    vec3 u_fogColor;
    float u_fogNear;
    float u_fogFar;
} u_fog;

layout(location = 18) uniform int u_pointLightCount;
layout(location = 19) uniform int u_spotLightCount;

#define MAX_LIGHTS 64

layout(location = 20) uniform struct PointLight {
...
// 340 = MAX_LIGHTS * 5 (# of fields in u_pointLights) + 15 (offset of u_pointLights)
layout(location = 340) uniform struct SpotLight {
    // Location of the light source
    vec3 position;
    // Direction
    vec3 direction;
...
{
    vec3 BumpMapNormal = texture2D(u_material.normalMap, v_UV).rgb * 2.0 - 1.0;
...
void main(void)
{
    ...
    vec3 normalMap = u_material.hasNormal ? CalcBumpedNormal() : v_surfaceNormal;
    ...
    float u_fogNear = 400.0;
    float u_fogFar = 1100.0;
    vec3 u_fogColor = vec3(1, 1, 1);

    float depth = gl_FragCoord.z / gl_FragCoord.w;
    float u_fogFactor = smoothstep(u_fogNear, u_fogFar, depth);
    gl_FragColor = mix(gl_FragColor, vec4(u_fogColor, gl_FragColor.w), u_fogFactor);
}